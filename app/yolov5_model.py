...
def detect_padi(image_path):
    # Melakukan deteksi dengan YOLOv5 tanpa menyimpan hasil
    results = run(weights='./models/best.pt', 
                  source=image_path, 
                  project=None,  # Tidak membuat folder proyek
                  save_txt=False,  # Tidak menyimpan file deteksi
                  save_conf=False,  # Tidak menyimpan confidence ke file
                  conf_thres=0.5, 
                  nosave=True)  # Tidak menyimpan hasil gambar deteksi

    # Parsing hasil deteksi langsung dari YOLOv5 output
    detections = results.xywhn[0].tolist()  # Mengambil hasil dalam format [class, x_center, y_center, width, height, confidence]
    ...
    for detection in detections:
        _, x_center, y_center, width, height, *_ = detection
        x1 = int((x_center - width / 2) * w)
        y1 = int((y_center - height / 2) * h)
        x2 = int((x_center + width / 2) * w)
        y2 = int((y_center + height / 2) * h)
    ...
    if largest_bbox:
        ...
        # Crop gambar berdasarkan bounding box terbesar
        cropped_image = image[y1:y2, x1:x2]
        print(f"Proses cropping selesai. Gambar diproses dalam memori.")
        return cropped_image  # Kembalikan hasil crop sebagai array gambar
